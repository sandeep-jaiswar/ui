...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({
    ...
    const variantStyles = {
      filled: 'bg-fill-secondary dark:bg-fill-secondary-dark border border-transparent focus:border-systemBlue-500 focus:bg-background-primary dark:focus:bg-background-tertiary-dark',
      outlined: 'bg-transparent border border-separator-opaque dark:border-separator-opaque-dark focus:border-systemBlue-500',
    ...
    return (
      <div className={`${fullWidth ? 'w-full' : ''} space-y-1`}>
        {label && (
          <label 
            htmlFor={props.id}
            className="block text-ios-subhead font-medium text-label-primary dark:text-label-primary-dark"
          >
            {label}
          </label>
        )}
        ...
        <div className="relative">
          {startIcon && (
            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-label-tertiary dark:text-label-tertiary-dark">
              {startIcon}
            </div>
          )}
            ...
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
              {showClear ? (
                <button
                  type="button"
                  onClick={handleClear}
                  className="text-label-tertiary dark:text-label-tertiary-dark hover:text-label-secondary dark:hover:text-label-secondary-dark transition-colors"
