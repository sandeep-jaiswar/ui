  ...
  ({ visible, title, message, actions = [], type = "alert", onClose, className = "", testId, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <div
          ref={ref}
          className={`animate-scale-in relative w-full max-w-sm overflow-hidden rounded-ios-xl bg-background-primary shadow-modal dark:bg-background-secondary-dark ${
            isActionSheet ? "mb-4" : ""
          } ${className} `.trim()}
          role="alertdialog"
          aria-modal="true"
              ...
              {actions.map((action, index) => {
                ...
                return (
                  <button
                    key={index}
                    onClick={() => handleActionPress(action)}
                    className={`flex-1 px-4 py-3 font-medium transition-colors text-ios-body ${
                      isDestructive
